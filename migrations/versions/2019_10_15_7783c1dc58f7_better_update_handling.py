...
def downgrade():
    op.add_column('update', sa.Column('time_window', postgresql.TIMESTAMP(), autoincrement=False, nullable=False))
    op.add_column('update', sa.Column('updated', sa.BOOLEAN(), autoincrement=False, nullable=False))
    op.add_column('update', sa.Column('count', sa.INTEGER(), autoincrement=False, nullable=False))
    op.create_unique_constraint('one_update_per_time_window', 'update', ['poll_id', 'time_window'])
